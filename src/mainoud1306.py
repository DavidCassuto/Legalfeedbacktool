...
def criteria_edit(criterion_id):
    """Bewerken van een bestaand criterium."""
    conn = get_db_connection()
    criterion = None # Zorg dat criterion altijd gedefinieerd is

    if conn: # Indentation level 1 (4 spaces)
        try: # Indentation level 2 (8 spaces)
            cursor = conn.cursor()

            if request.method == 'POST': # Indentation level 3 (12 spaces)
                name = request.form.get('name', '').strip()
                description = request.form.get('description', '').strip()
                category = request.form.get('category', 'Algemeen')
                color = request.form.get('color', '#F94144')
                ...
                if name: # Indentation level 4 (16 spaces)
                    cursor.execute( # Indentation level 5 (20 spaces)
                    ...
                    return redirect(url_for('criteria_list'))
                else: # Indentation level 4 (16 spaces)
                    flash('Naam is verplicht.', 'error')
            
            # Dit deel wordt uitgevoerd bij een GET-verzoek of bij een POST-fout (validatie)
            # Het blijft binnen de 'try' block, dus op het derde niveau (12 spaces)
            cursor.execute('SELECT * FROM criteria WHERE id=?', (criterion_id,))
            criterion = cursor.fetchone()

            if criterion is None: # Indentation level 3 (12 spaces)
                flash('Criterium niet gevonden.', 'error')
                return redirect(url_for('criteria_list'))

        except Exception as e: # Indentation level 2 (8 spaces), directly after the 'try' block's main logic
            print(f"Fout bij bewerken (GET/POST logic): {e}") # Indentation level 3 (12 spaces)
            flash('Er is een interne fout opgetreden bij het laden of opslaan van het criterium.', 'error') # Indentation level 3
            return redirect(url_for('criteria_list')) # Indentation level 3
        finally: # Indentation level 2 (8 spaces), matching 'try' and 'except'
            if conn: # Indentation level 3 (12 spaces)
                conn.close() # Indentation level 4 (16 spaces)
    else: # Indentation level 1 (4 spaces), matching 'if conn:'
        flash('Kan geen verbinding maken met de database.', 'error') # Indentation level 2 (8 spaces)
        return redirect(url_for('criteria_list')) # Indentation level 2 (8 spaces)

    # Deze 'return' stuurt de HTML-pagina. Hij staat op het EERSTE niveau (4 spaces)
    # Dit is de laatste executable lijn van de functie, en wordt alleen bereikt
    # als er geen redirects zijn geweest (bijv. bij een succesvolle GET).
    return render_template_string(BASE_TEMPLATE + '''
    <div class="header">
        <h1>✏️ Criterium Bewerken</h1>
        <p>Pas de details van een bestaand criterium aan.</p>
        <div class="nav-breadcrumb">
            <a href="/">Home</a> > <a href="/criteria">Criteria Beheer</a> > Criterium Bewerken
        </div>
    </div>
